...
CreateTorqueFromJointSpaceCallback(
  ...
  return [&global_handler, &state_publisher, &model, &current_state_info,
          &goal_state_info, &policy_rate,
          &traj_rate](const franka::RobotState &robot_state,
                      const franka::Duration period) -> franka::Torques {
    std::chrono::high_resolution_clock::time_point t1 =
        std::chrono::high_resolution_clock::now();
...
std::function<franka::Torques(const franka::RobotState &, franka::Duration)>
CreateTorqueFromHybridForcePositionCallback(
    const std::shared_ptr<SharedMemory> &global_handler,
    const std::shared_ptr<robot_utils::StatePublisher> state_publisher,
    const franka::Model &model, std::shared_ptr<StateInfo> &current_state_info,
    std::shared_ptr<StateInfo> &goal_state_info, const int &policy_rate,
    const int &traj_rate, const ForceSensorMessage &force_sensor_msg) {
  return [&global_handler, &state_publisher, &model, &current_state_info,
          &goal_state_info, &policy_rate, &traj_rate,
          &force_sensor_msg](const franka::RobotState &robot_state,
                             franka::Duration period) -> franka::Torques {
    std::chrono::high_resolution_clock::time_point t1 =
        std::chrono::high_resolution_clock::now();
    ...
    Eigen::Vector3d desired_wrench_in_sensor_frame;
    wrench_in_sensor_frame << force_sensor_msg.fx(), force_sensor_msg.fy(),
        force_sensor_msg.fz();
    desired_wrench_in_sensor_frame = goal_state_info->wrench_in_sensor_frame;
    ...
    global_handler->logger->info(
        "{}", spdlog::fmt_lib::join(tau_d_rate_limited, ","));
